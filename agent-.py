# agent.py
"""
GameForge AI - LangGraph agent (Human-in-the-loop)
Flow (exact order implemented):
  intent_analysis ->
  collect_user_idea ->
  generate_questions ->
  collect_user_answers ->
  validate_inputs ->
  design_game_blueprint ->
  engine_decision ->
  build_code_prompt ->
  generate_game_code ->
  review_code ->
...
Requirements:
- LANGGRAPH and langgraph.checkpoint.memory.MemorySaver available
- OpenAI SDK (pip install openai)
- A valid OPENAI_API_KEY in environment (or via .env)
- `interrupt` from langgraph.types is used to pause for human responses
"""
...
def llm_invoke_text(prompt: str) -> str:
    ...
    try:
        ...
        log_timestamp(f"❌ OpenAI API error: {str(e)}")
        raise
...
def design_game_blueprint(state: GameAgentState) -> GameAgentState:
    ...
    prompt = f"""
You are a senior game designer. Using the information below, produce a clear, structured design document for a production-quality 2D game(not top view).
...
═══════════════════════════════════════════════════════════
WORKING TEMPLATE - COPY THIS STRUCTURE EXACTLY
═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
// PRELOAD: Create Visible Sprites (DO NOT CHANGE THIS PATTERN)
// ═══════════════════════════════════════════════════════════
function preload() {{
    ...
    // Projectile sprite (yellow)
    g.fillStyle(0xffff00);
    g.fillCircle(8, 8, 8);
    g.generateTexture('projectile', 16, 16);
    g.clear();
    
    // Platform sprite (gray) - if needed
...
def review_code(state: GameAgentState) -> GameAgentState:
    ...
    log_timestamp(f"   - Contains <canvas>: {('<canvas' in code.lower())}")
    log_timestamp(f"   - Contains <script>: {('<script' in code.lower())}")
...
What I have given you are just examples of how to review - think and analyze the code for more potential errors. It does not need to be perfect but at least should not crash.
...
"""
    ...
    # Remove markdown code fences
    cleaned = re.sub(r'^```[a-zA-Z]*\s*', '', cleaned)  # remove starting code fence like ```python
    cleaned = re.sub(r'\s*```$', '', cleaned)            # remove ending code fence like ```
...
def apply_feedback_to_code(state: GameAgentState) -> GameAgentState:
    ...
    prompt = f"""
You are a game developer modifying a working browser game based on user feedback.

USER FEEDBACK:
"{feedback}"

CURRENT HTML GAME CODE:
{code}

Modify ONLY what's necessary to implement the feedback.
Do not break existing functionality.
Return the full fixed HTML file. No markdown fences, no explanations.
Start with <!DOCTYPE html>.
"""
...
workflow.add_edge("finalize_output", "collect_user_feedback")
